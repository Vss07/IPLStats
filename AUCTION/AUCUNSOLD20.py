...
def AUCUN2020():
    ...
    def default():
            ab=Ent.get()
            cur.execute("SELECT * from aucunsold2020")
            rows=cur.fetchall()
            update(rows)

    def bowl():
            ab=Ent.get()
            cur.execute("SELECT * from aucunsold2020 where Player LIKE '%"+ab+"%' and Type LIKE '%Bowl%' ")
            rows=cur.fetchall()
            update(rows)

    def bat():
            ab=Ent.get()
            cur.execute("SELECT * from aucunsold2020 where Player LIKE '%"+ab+"%' and Type LIKE '%Bat%' ")
            rows=cur.fetchall()
            update(rows)

    def allr():
            ab=Ent.get()
            cur.execute("SELECT * from aucunsold2020 where Player LIKE '%"+ab+"%' and Type LIKE '%All%'")
            rows=cur.fetchall()
            update(rows)

    def wk():
            ab=Ent.get()
            cur.execute("SELECT * from aucunsold2020 where Player LIKE '%"+ab+"%' and Type LIKE '%Keep%'")
            rows=cur.fetchall()
            update(rows)

    def price():
            ab=Ent.get()
            cur.execute("SELECT * from aucunsold2020 where Player LIKE '%"+ab+"%' order by length(Price) desc,Price desc;")
            rows=cur.fetchall()
            update(rows)
    ...
    def player():
            ab=Ent.get()
            cur.execute("SELECT * from aucunsold2020 where Player LIKE '%"+ab+"%'")
            player1()
