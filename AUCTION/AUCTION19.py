...
def AUC2019():
    ...
    def default():
            ...
            e=''
            ab=Ent.get()
            cur.execute("SELECT * from auction2019")
            rows=cur.fetchall()
            update(rows)

    def bowl():
            ab=Ent.get()
            cur.execute("SELECT * from auction2019 where Player LIKE '%"+ab+"%' and Type LIKE '%Bowl%' and Team LIKE '%"+e+"%' ")
            rows=cur.fetchall()
            update(rows)

    def bat():
            ab=Ent.get()
            cur.execute("SELECT * from auction2019 where Player LIKE '%"+ab+"%' and Type LIKE '%Bat%' and Team LIKE '%"+e+"%' ")
            rows=cur.fetchall()
            update(rows)

    def allr():
            ab=Ent.get()
            cur.execute("SELECT * from auction2019 where Player LIKE '%"+ab+"%' and Type LIKE '%All%' and Team LIKE '%"+e+"%'")
            rows=cur.fetchall()
            update(rows)

    def wk():
            ab=Ent.get()
            cur.execute("SELECT * from auction2019 where Player LIKE '%"+ab+"%' and Type LIKE '%Keep%' and Team LIKE '%"+e+"%'")
            rows=cur.fetchall()
            update(rows)

    def price():
            ab=Ent.get()
            cur.execute("SELECT * from auction2019 where Player LIKE '%"+ab+"%' and Team LIKE '%"+e+"%' order by length(Price) desc,Price desc;")
            rows=cur.fetchall()
            update(rows)
    ...
    def player():
            ab=Ent.get()
            cur.execute("SELECT * from auction2019 where Player LIKE '%"+ab+"%'")
            player1()
    ...
    def CSK():
            ...
            ab=Ent.get()
            cur.execute("SELECT * from auction2019 where Team LIKE '%Chennai%'")
            rows=cur.fetchall()
            update(rows)
    ...
    def MI():
            ...
            e='Mumbai Indians'
            ab=Ent.get()
            cur.execute("SELECT * from auction2019 where Team LIKE '%Mumbai%'")
            rows=cur.fetchall()
            update(rows)
    ...
    def DC():
            ...
            e='Delhi Capitals'
            ab=Ent.get()
            cur.execute("SELECT * from auction2019 where Team LIKE '%Delhi%'")
            rows=cur.fetchall()
            update(rows)
    ...
    def KKR():
            ...
            ab=Ent.get()
            cur.execute("SELECT * from auction2019 where Team LIKE '%Kolkata%'")
            rows=cur.fetchall()
            update(rows)
    ...
    def KXIP():
            ...
            e='Kings XI Punjab'
            ab=Ent.get()
            cur.execute("SELECT * from auction2019 where Team LIKE '%Punjab%'")
            rows=cur.fetchall()
            update(rows)
    ...
    def RR():
            ...
            e='Rajasthan Royals'
            ab=Ent.get()
            cur.execute("SELECT * from auction2019 where Team LIKE '%Rajasthan%'")
            rows=cur.fetchall()
            update(rows)
    ...
    def RCB():
            ...
            ab=Ent.get()
            cur.execute("SELECT * from auction2019 where Team LIKE '%Bangalore%'")
            rows=cur.fetchall()
            update(rows)
    ...
    def SRH():
            ...
            ab=Ent.get()
            cur.execute("SELECT * from auction2019 where Team LIKE '%Hyderabad%'")
            rows=cur.fetchall()
            update(rows)
